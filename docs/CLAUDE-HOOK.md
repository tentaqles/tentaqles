...
Any other fields in the payload (tool name, session id, etc.) are ignored.
...
results instead of scraping text. `claude-hook` itself does not take a
`--json` flag — its output is the fixed preamble/block-message format
described above, since it feeds Claude Code's hook UI, not a script.
...
3. **`neutral-remote`** — cwd resolves to **no** trusted tq workspace
   ...
   commands).
4. **`untrusted`** — git only. The workspace resolved but isn't trusted
   (`tq allow <name>` hasn't been run, or the manifest changed since).
5. **`env-drift`** — git only. The shell's exported `TQ_WS` doesn't match
...
6. **`claude-config-drift`** — remote mutation (`git push`/`gh`) only. The
   ...
   `CLAUDE_CONFIG_DIR`.
7. **`git-email-drift`** — non-read-only git only, and only when both the
...
subject to the email check. This exemption applies only to rule 7; rules
1–6 and 8 are unaffected by whether the git command is read-only.
...
```sh
echo '{"cwd":"'$PWD'","tool_input":{"command":"git push"}}' | tq claude-hook pre-tool-use; echo $?
```

From PowerShell:

```powershell
'{"cwd":"' + ($PWD.Path -replace '\\','\\\\') + '","tool_input":{"command":"git push"}}' | tq claude-hook pre-tool-use; $LASTEXITCODE
```
...
```sh
echo '{"cwd":"'$PWD'"}' | tq claude-hook session-start
```
